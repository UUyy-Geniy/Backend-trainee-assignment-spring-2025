    ...
    async def get_pvz_list(
        ...
        async with self._uow.atomic():
            ...
            receptions_subq = (
                ...
                .select_from(receptions.outerjoin(products_subq, receptions.c.id == products_subq.c.reception_id))
                ...
                select(
                    ...
                    func.coalesce(
                        func.jsonb_agg(
                            func.jsonb_build_object(
                                "id",
                                receptions_subq.c.id,
                                "date_time",
                                receptions_subq.c.date_time,
                                "status",
                                receptions_subq.c.status,
                                "pvz_id",
                                receptions_subq.c.pvz_id,
                                "products",
                                receptions_subq.c.products,
                            )
                        ).filter(receptions_subq.c.id.is_not(None)),
                        empty_json_array,
                    ).label("receptions"),
                )
                .select_from(pvz.outerjoin(receptions_subq, pvz.c.id == receptions_subq.c.pvz_id))
                .group_by(pvz.c.id)
            )

            if start_date and end_date:
                query = query.where(pvz.c.registration_date.between(start_date, end_date))
