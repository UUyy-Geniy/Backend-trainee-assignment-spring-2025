...
class ProductRepository(BaseRepository[Table]):
    ...
    async def get_last_product(self, reception_id: str) -> Optional[dict]:
        query = (
            select(self.table)
            .where(self.table.c.reception_id == reception_id)
            .order_by(self.table.c.removal_order.desc())
            .limit(1)
        )
    ...
    async def get_last_active_product(self, reception_id: str) -> Optional[dict]:
        query = (
            select(self.table)
            .where((self.table.c.reception_id == reception_id) & (self.table.c.removed == False))
            .order_by(self.table.c.removal_order.desc())
            .limit(1)
        )
        result = await self.conn.execute(query)
