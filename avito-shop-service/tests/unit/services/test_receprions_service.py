...
@pytest.mark.parametrize(
    "case",
    test_start_reception_cases,
    ids=[c["name"] for c in test_start_reception_cases],
)
async def test_start_reception(mock_uow, case):
    ...
    if case["active_reception"]:
        ...
        mock_uow.receptions.create.return_value = {
            "id": "new_rec",
            "status": "in_progress",
        }
    ...
    if case["expected_error"]:
        ...
        mock_uow.receptions.create.assert_awaited_once_with(pvz_id=case["pvz_id"], status="in_progress")
...
@pytest.mark.parametrize(
    "case",
    test_close_reception_cases,
    ids=[c["name"] for c in test_close_reception_cases],
)
async def test_close_last_reception(mock_uow, case):
    ...
    if case["active_reception"]:
        mock_uow.receptions.update.return_value = {
            "id": case["active_reception"]["id"],
            "status": "close",
        }
    else:
    ...
    if case["expected_error"]:
        ...
        assert result["status"] == case["expected_status"]
        mock_uow.receptions.get_active_reception.assert_awaited_once_with(case["pvz_id"])
        mock_uow.receptions.update.assert_awaited_once_with(case["active_reception"]["id"], status="close")
